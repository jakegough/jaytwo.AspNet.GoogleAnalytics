...
// filter taken from glimpse project (modified to remove logging, and to take a delegate for the html snippet)
//   https://github.com/Glimpse/Glimpse/blob/master/source/Glimpse.AspNet/PreBodyTagFilter.cs
//   https://github.com/Glimpse/Glimpse/blob/7f9e798a1430da86821b66a3866eb15b04347950/source/Glimpse.AspNet/PreBodyTagFilter.cs
...
namespace jaytwo.AspNet.GoogleAnalytics
{
    ...
    public class PreBodyTagFilter : Stream
    {
        ...
        private Func<string> HtmlSnippetDelegate { get; set; }
		...
		public PreBodyTagFilter(string htmlSnippet, Stream outputStream, Encoding contentEncoding)
			: this(() => htmlSnippet, outputStream, contentEncoding)
		{
		}

        public PreBodyTagFilter(Func<string> htmlSnippetDelegate, Stream outputStream, Encoding contentEncoding)
        {
			HtmlSnippetDelegate = htmlSnippetDelegate;
		...
		private string GetHtmlSnippet()
		{
			var htmlSnippet = HtmlSnippetDelegate.Invoke();
			var result = htmlSnippet + BodyClosingTag;
			return result;
		}
        ...
        public override void Flush()
        {
            if (!string.IsNullOrEmpty(UnwrittenCharactersFromPreviousCall))
            {
                ...
                if (BodyEndRegex.IsMatch(UnwrittenCharactersFromPreviousCall))
                {
					...
					var htmlSnippet = GetHtmlSnippet();
